...
## 2026-09-13 — PWA installability + Kiswahili toggle (6.1)
- PWA: manifest.webmanifest, sw.js (31-file precache, offline.html fallback, runtime caches), pwa.js (SW register + install prompt), header install button (hidden until beforeinstallprompt), generated icons (192/512/apple-touch, pure-python PNG, no rsvg).
- i18n: i18n.js (EN/SW dict ~170 keys, t/tf, selector+textMap+firstText/lastText/html rules, setLang + __rerenderLang hooks), header SW/EN toggle, appLocale() dates, timeAgo SW, verdict/compare/story/MC/pred/hub/story labels SW.
- Scope notes: article narratives + legal body prose stay EN (chrome only per 6.1); story reason keeps pick-time language on toggle; ordinals (1st/2nd) EN; install prompt needs real-device test.
- Smoke 33/33 vm + 19/19 static + 10/10 serve.
